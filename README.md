...
| `js/DebugLog.js` | 画面に診断ログを出す。表示するかは `ENABLED_BY_DEFAULT` と `?debug=` で決まる |
...
iOS Safari は手元でコンソールが見られないため、画面下に診断ログを出せるようにしています。
**不具合の切り分け中なので、いまは URL に何も付けなくても表示されます（既定 ON）。**

```
http://<サーバの IP>:8000/            既定で表示される
http://<サーバの IP>:8000/?debug=0    非表示にする
http://<サーバの IP>:8000/?debug=1    明示的に表示する（従来どおり）
```

`?debug=0`（`off` / `false` / `no` も可）で非表示、`?debug=1`（`on` / `true` / `yes` も可）で表示。
指定がないときの既定値は `js/DebugLog.js` の **`ENABLED_BY_DEFAULT` という定数 1 つ**で決まります。
切り分けが済んで既定 OFF に戻すときは、この定数を `false` にするだけで済みます（他の場所は触りません）。

見出し「▼ 診断ログ」をタップするとパネルを折りたためます。パネルは通常のフロー（`position: static`）に
置かれているため、タイルや音量スライダーを覆いません。

記録されるのは次の 4 種です。無効なときは何も表示せず、ログも記録しません。
